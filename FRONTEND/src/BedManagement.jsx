  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
        <div>
          <h2 className="text-3xl font-bold text-[var(--text-main)] tracking-tight mb-2">Rooms & Beds</h2>
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button className="premium-button bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-muted)] hover:bg-[var(--bg-main)] flex items-center justify-center gap-2 flex-1 sm:flex-none">
          ...
          <div key={i} className="premium-card p-6 group">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <span className="text-2xl font-bold text-[var(--text-main)] leading-none">{stat.value}</span>
                <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">{stat.label}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded-full w-fit transition-all duration-300 group-hover:scale-105">
      ...
      <div className="premium-card p-6 lg:p-8">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-1 bg-[var(--bg-main)] p-1 rounded-2xl border border-[var(--border-color)] overflow-x-auto no-scrollbar w-full lg:w-auto">
            {wards.map(ward => (
              <button
                key={ward}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {filteredBeds.map(bed => (
            <div key={bed.id} className={`relative p-6 rounded-[24px] border-2 transition-all duration-300 group ${
              bed.status === 'Occupied' ? 'bg-orange-500/10 border-orange-500/20' : 
              bed.status === 'Cleaning' ? 'bg-blue-500/10 border-blue-500/20' :
              bed.status === 'Maintenance' ? 'bg-rose-500/10 border-rose-500/20' :
              'bg-[var(--bg-card)] border-[var(--border-color)] hover:border-emerald-500 hover:shadow-xl hover:shadow-emerald-500/10'
            }`}>
              <div className="flex flex-col items-center gap-4">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                  ...
                  'bg-[var(--bg-main)] text-[var(--text-muted)] group-hover:bg-emerald-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-emerald-500/20'
                }`}>
                  <Bed size={28} />
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-[var(--text-main)] tracking-tight">{bed.id}</p>
                  <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">{bed.type}</p>
                </div>
                <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 transition-all duration-300 ${
                  ...
                  {bed.status === 'Available' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse transition-all"></span>}
                  {bed.status}
                </div>
              </div>
              ...
              {bed.patient && (
                <div className="mt-4 pt-3 border-t border-[var(--border-color)]">
                  <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-tighter mb-1 text-center">Assigned Patient</p>
                  <p className="text-xs font-bold text-[var(--text-main)] text-center truncate">{bed.patient}</p>
                </div>
              )}
            </div>
          ))}
